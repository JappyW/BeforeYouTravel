...
import { CountriesPathsSelection, CountryFeature, GlobeProps, InitialGlobeProps, SVGDatum } from "../../types"
import { geoOrthographic, geoPath, select } from 'd3';
import { dragProjection, rotateProjectionTo, zoomProjection } from "../../hooks/useTransformations";
import { Country, GlobeContainer, GlobeSVG, Ocean } from "../../styles/Globe";

const initialWidth = window.innerHeight - 220;

const initialValues: InitialGlobeProps = {
    rotation: [0, 0],
    width: initialWidth,
    height: initialWidth,
...
const Globe = (props: GlobeProps) => {
    const {
        ...
        minScroll = initialValues.minScroll,
        maxScroll = initialValues.maxScroll,
        dragSensitivity = initialValues.dragSensitivity,
        selectedCountry,
        countries,
        handleCountryClick,
    } = props;
    ...
    const projection = useMemo(() => {
        return geoOrthographic().scale(scale).center([0, 0]).rotate(rotation).translate([width / 2, height / 2]);
    }, [scale, width, height])
    ...
    useEffect(() => {
        ...
        const countryPaths: CountriesPathsSelection = select(svgRef.current).selectAll('path');
