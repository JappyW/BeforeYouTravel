...
import { CountryFeature, ICountryInfoFull, Rotation } from '../../types';
import {
  AppBar,
  Autocomplete,
  Button,
...
export const MainPage = () => {
  ...
  const handleCountrySelect = (countryName: string | null) => {
    ...
    const name = countries.find(c => c.properties.name === countryName)!;

    setSelectedCountry(name);
    setRotation(name.properties.position);
    openDrawer();
  };

  const handleCountryClick = (name: CountryFeature) => {
    setSelectedCountry(name);
    setRotation(name.properties.position);
    openDrawer();
  };
      ...
      <Grid container padding={5} direction='column'>
        ...
        >
          <Grid
            ...
            sx={{ maxHeight: '50px', padding: '20px' }}
          >
          ...
          <CountryInfo
            id='1'
            name={selectedCountry?.properties.name as CountryName}
          />
        </Drawer>
        <Grid item xs={3}>
          <Autocomplete
